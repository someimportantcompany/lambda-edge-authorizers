    ...
    headers: {
      ...(res?.cookies && {
        'set-cookie': Object.entries(res.cookies).map(([name, { value, ...opts }]) => {
          ...
          const expires =
            value !== null && expiresInSecs !== undefined ? new Date(Date.now() + expiresInSecs) : undefined;
          const maxAge = value !== null && expiresInSecs !== undefined ? Math.floor(expiresInSecs / 1000) : undefined;
